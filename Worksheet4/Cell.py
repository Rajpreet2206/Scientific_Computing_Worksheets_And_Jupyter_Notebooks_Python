...
class Cell():
    ...
    def __init__(self, n_crit, center=[0.5, 0.5], side_length=1., M=3):
        ...
        self.inner_ring_expansion_terms = np.zeros(K, dtype=float) # potential at quadrature points due to particles outside the box
        self.outer_ring_expansion_terms = np.zeros(K, dtype=float) # potential at quadrature points due to particles inside the box
        self.kappa = None
